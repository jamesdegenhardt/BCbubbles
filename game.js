...
const timedMatchClock = document.getElementById('timedMatchClock');
const timedMatchClockValue = document.getElementById('timedMatchClockValue');
...
const teamMassValues = { Red: document.getElementById('redMass'), Green: document.getElementById('greenMass'), Blue: document.getElementById('blueMass') };
...
const BOT_NAMES = ['BubbleBurst_BC', 'PopMasterBC', 'SoapSudSlayer', 'SupaSoaper', 'FizzyFloater', 'GlindaTheBubble', 'TheBubbleBoss', 'AquaSphere_BC', 'BubbleTrouble', 'PopCzar', 'BC_Blaster', 'Captain_BC', 'BC_Breeze', 'BaronVonBC', 'BC_Bomber', 'Bamboozled', 'WorstNameEver', 'U_WOT_M8', 'EmotionalDamage', 'ThisIsFine', 'ToughCookie', 'WowBoom', 'RogueWarrior', 'ShockwaveGamma', 'Vertigo', 'DarkMechanic', 'Cascade', 'Rotor', 'SudsySamurai', 'BC_Bouncy', 'PopStarBC', 'FoamFighter', 'Carbonated', 'BC_Ballistic', 'OrbisPop', 'SoapOpera', 'SphericalBC', 'BubbleWrapp', 'AirborneBC', 'SudsSurfer', 'BC_Blizzard', 'Effervescent', 'PopNLock', 'BC_Buoyancy', 'LatherKing', 'AeroBC', 'FizzyWizzy', 'BC_Bullet', 'GamerSoap', 'PopTastic', 'BC_Baron', 'BubbleByte', 'FloatyMcFloat', 'SudsySlick', 'BC_Breaker', 'PopShot', 'AquaBC', 'FoamZone', 'BurstBrigade', 'BC_Blitz', 'SodaPopBC', 'BubbleBash', 'SudsGod', 'BC_Cyclone', 'PopFrenzy', 'NebulaBubble', 'BC_Glider', 'SoapStruck', 'AirBubble', 'BC_Striker', 'PopGamer', 'SudStorm', 'BC_Vortex', 'BubbleWave', 'LatherLord', 'BC_Titan', 'PopQuest', 'FizzyForce', 'BC_Echo', 'BubbleNova', 'BubbleKnight_BC', 'SudsSniper', 'PopCommander', 'Iambad', 'SoapScrubber', 'FoamyPhantom', 'BC_Beacon', 'YourMom', 'BurstMechanic', 'Ibeatyou', 'BC_Boundless', 'irhguirh', 'lollolol', 'BubbleBro', 'BC_Cruiser', 'HyperBubble', 'SudsSurge', 'BC_Banshee', 'U_suck', 'AquaAura_BC', 'BubbleDrifter', 'LatherLegion', 'BC_Grizzly', 'PopViper', 'SoapSlinger', 'Sorry', 'BC_Dynamo', 'SudsySultan', 'FizzyFighter', 'teamwithme', 'PopCatalyst', 'BubbleJumper', 'SoapSovereign', 'BC_Horizon', 'AeroBurst', 'SudsSpecter', 'BC_Avenger', 'PopDynamite', 'BubbleWizard', 'SoapShogun', 'BC_Meteor', 'FizzyFrenzy', 'PopGoliath', 'BubbleRider', 'BC_Comet', 'SudsySailor', 'LatherLogic', 'BC_Apex', 'PopBarrage', 'BubbleGlitch', 'SoapShadow', 'BC_Ranger', 'FizzyFusion', 'PopWarden', 'BubbleSpark', 'BC_Summit', 'SudsySiren', 'SoapSpartan', 'BC_Phantom', 'PopRumble', 'BubbleChaser', 'LatherLancer', 'BC_Viper', 'FizzyGamer', 'PopTornado', 'BubbleShield', 'BC_Rogue', 'SudsyStriker', 'SoapSentry', 'BC_Pulse', 'PopEclipse', 'BubbleVoyager', 'LatherLegend', 'BC_Specter', 'FizzyFlash', 'PopHavoc', 'BubbleBreaker', 'BC_Zenith', 'SudsyScout', 'SoapSoldier', 'BC_Matrix', 'PopVortex', 'BubblePioneer', 'LatherLink', 'BC_Quake', 'FizzyFalcon', 'PopTitanium', 'BubbleRebel', 'BC_Saber', 'SudsyShadow', 'SoapSystem', 'BC_Nexus', 'PopQuake', 'BubbleRival', 'LatherLead', 'BC_Omega', 'FizzyFury', 'PopOverlord', 'BubbleTracker', 'BC_Alpha'];
const food = [];
...
let worldSize = 'normal';
let botTarget = BOT_TARGET;
...
function respawnTimedOwners() { if (gameMode !== 'timed' || gameState !== 'playing') return; for (const owner of [player, ...bots]) if (!ownedCells(owner).length) { const spawn = spawnPoint(260); const cell = createCell(owner, spawn.x, spawn.y, 12); if (owner === player) { player.controlledCell = cell; selectedCell = null; } } }
function setupBots() {
  bots.length = 0;
...
function updateMothercells(delta) { if (gameMode !== 'experimental') return; for (const mother of mothercells) { mother.timer -= delta; mother.pulse += delta * 2; if (mother.timer <= 0) mother.timer = 1.4; for (const cell of cells.slice()) if (cell.radius < mother.radius && distanceBetween(cell, mother) < mother.radius * 1.2) { emitBurst(cell.x, cell.y, '#ff75d4', 18, 200); addFloatingText(cell.x, cell.y, 'MOTHERCELL', '#ff75d4'); removeCell(cell); if (cell.owner === player && gameMode !== 'timed') endGame('loss'); } } }
...
function teamMass(teamName) { return ownedCells(player).filter((cell) => player.team === teamName).reduce((sum, cell) => sum + cell.targetMass, 0) + bots.filter((bot) => bot.team === teamName).reduce((sum, bot) => sum + ownedCells(bot).reduce((botMass, cell) => botMass + cell.targetMass, 0), 0); }
function resolveTeamWinner() { const livingTeams = TEAMS.filter((team) => teamMass(team.name) > 0); if (livingTeams.length > 1) return; if (livingTeams.length === 1) endGame(player.team === livingTeams[0].name ? 'win' : 'loss'); }
...
function update(delta, now) {
  ...
  if (gameState === 'playing' && gameMode === 'timed' && (!Number.isFinite(match.durationMinutes) || match.durationMinutes <= 0)) { match.durationMinutes = Number(timedDurationSelect.value) || 15; match.startedAt = performance.now(); }
...
function updateUi(totalMass, gasPhase = { phase: 'safe', remaining: 0 }) {
  ...
  matchTimeLine.hidden = gameMode !== 'timed'; matchTimeValue.textContent = timedClock; timedMatchClock.hidden = gameMode !== 'timed'; timedMatchClockValue.textContent = timedClock;
...
splitButton.addEventListener('pointerdown', (event) => { event.preventDefault(); splitPlayer(); });
