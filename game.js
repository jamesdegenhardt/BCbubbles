...
function randomFood(gridX, gridY) { const legendary = Math.random() < .025; const x = gridX === undefined ? Math.random() * WORLD.width : gridX * GRID_SIZE + 12 + Math.random() * (GRID_SIZE - 24); const y = gridY === undefined ? Math.random() * WORLD.height : gridY * GRID_SIZE + 12 + Math.random() * (GRID_SIZE - 24); return { x: clamp(x, 12, WORLD.width - 12), y: clamp(y, 12, WORLD.height - 12), radius: legendary ? 11 : 5 + Math.random() * 4, value: legendary ? 12 : 1, legendary, color: legendary ? '#ffd34e' : `hsl(${Math.floor(Math.random() * 360)} 90% 67%)` }; }
function foodInGrid(gridX, gridY) { return food.filter((pellet) => Math.floor(pellet.x / GRID_SIZE) === gridX && Math.floor(pellet.y / GRID_SIZE) === gridY).length; }
...
function resetFood() { food.length = 0; for (let gridX = 0; gridX < Math.ceil(WORLD.width / GRID_SIZE); gridX += 1) for (let gridY = 0; gridY < Math.ceil(WORLD.height / GRID_SIZE); gridY += 1) { const centerDistance = Math.hypot(gridX + .5 - WORLD.width / GRID_SIZE / 2, gridY + .5 - WORLD.height / GRID_SIZE / 2); const count = centerDistance < Math.min(WORLD.width, WORLD.height) / GRID_SIZE * .3 ? 3 + Math.floor(Math.random() * 2) : 1 + Math.floor(Math.random() * 4); for (let foodIndex = 0; foodIndex < count; foodIndex += 1) food.push(randomFood(gridX, gridY)); } }
resetFood();
...
function updateMothercells(delta) { if (gameMode !== 'experimental') return; for (const mother of mothercells) { mother.timer -= delta; mother.pulse += delta * 2; if (mother.timer <= 0) { mother.timer = 1.4; const gridX = Math.floor(mother.x / GRID_SIZE); const gridY = Math.floor(mother.y / GRID_SIZE); if (foodInGrid(gridX, gridY) < 4) { const angle = Math.random() * Math.PI * 2; food.push({ x: clamp(mother.x + Math.cos(angle) * (mother.radius + 18), 20, WORLD.width - 20), y: clamp(mother.y + Math.sin(angle) * (mother.radius + 18), 20, WORLD.height - 20), radius: 6, color: '#ff9be8' }); } } for (const cell of cells.slice()) if (cell.radius < mother.radius && distanceBetween(cell, mother) < mother.radius * 1.2) { emitBurst(cell.x, cell.y, '#ff75d4', 18, 200); addFloatingText(cell.x, cell.y, 'MOTHERCELL', '#ff75d4'); removeCell(cell); if (cell.owner === player) endGame('loss'); } } }
...
function applyFood(cell) {
  for (let index = food.length - 1; index >= 0; index -= 1) {
    ...
    if (distanceBetween(cell, pellet) < cell.radius + pellet.radius) { food[index] = randomFood(Math.floor(pellet.x / GRID_SIZE), Math.floor(pellet.y / GRID_SIZE)); cell.targetMass += pellet.value; if (cell.owner === player) { player.eaten += 1; match.food += 1; } emitBurst(pellet.x, pellet.y, pellet.color, pellet.legendary ? 12 : 4, pellet.legendary ? 130 : 55); playSound('eat', pellet.legendary ? 1 : .45); addFloatingText(cell.x, cell.y, pellet.legendary ? `+${pellet.value} LEGENDARY` : `+${pellet.value}`, pellet.legendary ? '#ffd34e' : '#a8f36d'); }
  }
}
